...
func _physics_process(_delta: float) -> void:
	...
	if (abs(interpolated_rotation.x - new_rotation.x) < 0.001
		and abs(interpolated_rotation.y - new_rotation.y) < 0.001
		and abs(interpolated_rotation.z - new_rotation.z) < 0.001):
